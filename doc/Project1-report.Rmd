...
short500.tokenized.joy <- tokenized.word.joy %>% count(word, sort = TRUE) %>% head(500)
```
...
#plot(frequent_terms) #combined look of frequent words
...
## What comment out:
## Time-consuming code commented out & result provided by load():
...
#write.csv(ldaOut.topics,file=paste("../output/LDAGibbs",k,"SongsToTopics.csv"))
...
# Based on the most frequent stemmed word and also referenced to the Topic Probability from the LDA process, I manully assign a hashtag to the generated Topic as following.
topics.hash=c("Topic1", "Topic2 = Love", "Topic3", "Topic4", "Topic5", "Topic5", "Topic2")

topics.hash=c("Family", "Youth", "Love", "Party", "State of Mind", "Outdoor", "Past&Future")
...
R&B and Jazz in term of happy songs, they have similar lyrics repetition ratio. On average, R&B songs has lightly over 47.2% of repetition, and Jazz music has 40.4% of repetition. R&B songs are more repetitive and easy to remember as a result of high repetitiveness. And thus it is easy to become popular. This could be used to support why R&B is [far more popular](https://www.statista.com/statistics/310746/share-music-album-sales-us-genre/) than Jazz in the current music market.
...
3. For happy songs, R&B lyrics are more repetitive than Jazz lyrics, which aids memory and popularity of the songs. This could probably be one of the reasons that R&B is more popular than Jazz.
